import { firebaseConfig, ADMIN_EMAILS, COURSES, LAST_UPDATED } from './firebase-config.js';
...
if (footerUpdatedEl) footerUpdatedEl.textContent = LAST_UPDATED;
...
const els = {
  ...
  exportSubsBtn: document.getElementById("exportSubsBtn"),
};
...
els.loginBtn.addEventListener("click", async () => {
  ...
  catch (err) { console.error(err); els.loginError.textContent = "Sign-in failed. Please try again."; els.loginError.classList.add("show"); }
});
...
els.tabSubscribers.addEventListener("click", () => switchTab("subscribers"));
...
function switchTab(tab) {
  ...
  els.tabSubscribers.classList.toggle("active", tab === "subscribers");
  ...
  els.subscribersPanel.style.display = tab === "subscribers" ? "block" : "none";
...
els.postAnnBtn.addEventListener("click", async () => {
  ...
  const kind = [...els.annKind].find(r => r.checked)?.value || "info";
  if (!title) { els.annError.textContent = "Please add a title."; els.annError.classList.add("show"); return; }
  els.postAnnBtn.disabled = true;
  try {
    await addDoc(collection(db, "announcements"), { title, body: body || null, link: link || null, kind, createdAt: serverTimestamp() });
    els.annTitle.value = ""; els.annBody.value = ""; els.annLink.value = "";
    await loadAnnouncements();
  } catch (err) { console.error(err); els.annError.textContent = "Failed to post. Please try again."; els.annError.classList.add("show"); }
  finally { els.postAnnBtn.disabled = false; }
});
...
async function loadAnnouncements() {
  ...
  if (items.length === 0) { els.annList.innerHTML = `<div class="empty-state">No announcements yet.</div>`; return; }
  els.annList.innerHTML = items.map(a => {
    ...
    return `<div class="announce-card" style="border-top-color:transparent;">
      <span class="ann-badge ${kind}" style="margin-bottom:8px; display:inline-block;">${kind}</span>
...
async function loadSubscribers() {
  ...
  els.subCount.textContent = subscribersCache.length;
  if (subscribersCache.length === 0) { els.subList.innerHTML = `<div class="empty-state">No subscribers yet.</div>`; return; }
  els.subList.innerHTML = subscribersCache.map(s => {
    const dateStr = s.createdAt?.toDate ? s.createdAt.toDate().toLocaleDateString("en-IN", { year: "numeric", month: "short", day: "numeric" }) : "";
    return `<div class="sub-row"><span>${escapeHtml(s.email || "")}</span><span style="color:var(--text-400); font-size:12px;">${dateStr}</span></div>`;
  }).join("");
}
els.exportSubsBtn.addEventListener("click", () => {
  if (subscribersCache.length === 0) return;
  const csv = "email,date\n" + subscribersCache.map(s => {
    const dateStr = s.createdAt?.toDate ? s.createdAt.toDate().toISOString() : "";
    return `${s.email},${dateStr}`;
  }).join("\n");
...
function renderList(container, items, isPending) {
  if (items.length === 0) { container.innerHTML = `<div class="empty-state">Nothing here.</div>`; return; }
  container.innerHTML = items.map(m => {
    const href = m.type === "Question Papers" ? (m.pdfLink || (m.pageLinks && m.pageLinks[0]) || "#") : (m.fileURL || m.link || "#");
    const dateStr = m.createdAt?.toDate ? m.createdAt.toDate().toLocaleDateString("en-IN", { year: "numeric", month: "short", day: "numeric" }) : "—";
    const qpInfo = m.type === "Question Papers" ? ` · ${m.paperFormat === "images" ? (m.pageLinks || []).length + " page images" : "PDF"}` : "";
    return `<div class="material-card">
      <div class="body">
        ...
        <div class="path">${pathLabel(m)}${qpInfo}</div>
      </div>
      ...
      <div class="actions">
        ...
        ${isPending
          ? `<button class="btn btn-ok btn-sm" data-action="approve" data-id="${m.id}">Approve</button><button class="btn btn-danger btn-sm" data-action="reject" data-id="${m.id}">Reject</button>`
          : `<button class="btn btn-danger btn-sm" data-action="delete" data-id="${m.id}">Remove</button>`}
      </div>
    </div>`;
  }).join("");
  container.querySelectorAll("button[data-action]").forEach(btn => btn.addEventListener("click", () => handleAction(btn.dataset.action, btn.dataset.id)));
}

async function handleAction(action, id) {
  const ref = doc(db, "materials", id);
  if (action === "approve") await updateDoc(ref, { status: "approved" });
  else if (action === "reject" || action === "delete") { if (!confirm("Are you sure? This can't be undone.")) return; await deleteDoc(ref); }
  await loadAll();
}
